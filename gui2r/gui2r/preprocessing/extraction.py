...
class Extractor(object):
    ...
    # Pattern to extract the full activity name
    # Examples:
    # "se.perigee.android.seven/com.perigee.seven.ui.activity.WorkoutSessionActivity"
    # "activity_name": "com.scotiabank.mobile/mobi.scotiabank.mobilehelp.activities.MobileHelpActivity",
    JSONPATH_EXPR_ACTIVITY_NAME = parse('$..activity_name')
    # Pattern to extract resource-ids
    # Examples:
    # "android:id/action_mode_bar_stub"
    # "se.perigee.android.seven:id/decor_content_parent"
    # "android:id/content"
    # "com.scotiabank.mobile:id/listViewQuestions"
    # "com.scotiabank.mobile:id/mobileHelp_entryText"
    ...
    def load_dataset(self, conf: Configuration, force: Optional[bool] = False) -> pd.DataFrame:
        ...
        if force or (not os.path.isfile(preproc_dataset_path)):
            ...
            # After loading the data we print the stats
            return dataframe
        else:
    ...
    def text_extraction_from_file(self, file_path_combined: Text, file_path_semantic: Text,
            ...
            # Filter apps that belong to a certain category directly
            if conf.filter_guis:
            ...
            with open(file_path_combined + file_name, 'r', encoding='utf8') as file_1:
                ...
                if conf.filter_guis:
                    # Text used in language detection is the visible AND the inivisible text
...
if __name__ == '__main__':
    camel_case_split = Extractor.camel_case_split('statusBarBackground')
    print('CamelCase-Split : {}'.format(camel_case_split))

    snake_case_split = Extractor.snake_case_split('status_Bar_Background')
    print('SnakeCase-Split : {}'.format(snake_case_split))

    snake_camel_case_split = Extractor.snake_camel_case_split('statusBarBackground_bar1_StableTest_results')
    print('SnakeCamelCaseSplit : {}'.format(snake_camel_case_split))

    snake_camel_case_split = Extractor.snake_camel_case_prob_split('statusBarBackground_bar1_StableTestcreatenewprofile_results')
    print('SnakeCamelCaseProbSplit : {}'.format(snake_camel_case_split))

    snake_camel_case_split = Extractor.snake_camel_case_prob_split('statusBarBackground_bar1_StableTestcasdasiasdas_results')
    print('SnakeCamelCaseProbSplit : {}'.format(snake_camel_case_split))

    norm_rid = Extractor.normalize_resource_id('android:id/navigationBarBackground_test')
    print('Normalized rid : {}'.format(norm_rid))

    norm_rid = Extractor.normalize_resource_id('android:id/product_originalPrice')
    print('Normalized rid : {}'.format(norm_rid))

    norm_act_id = Extractor.normalize_activity_name_2('com.about.CalorieCount/com.about.CalorieCount.v2.Activities.FoodActivity',
                                                      filter_tokens=['Activity', 'Activities', 'Main', 'com'])
    print('Normalized aid : {}'.format(norm_act_id))

    norm_act_id = Extractor.normalize_activity_name_2(
        'com.asynchrony.emerson.sensi/com.emerson.sensi.register.RegisterActivity',
        filter_tokens=['Activity', 'Activities', 'Main', 'com'])
    print('Normalized aid : {}'.format(norm_act_id))